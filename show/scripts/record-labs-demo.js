async (page) => {
  const outputPath = process.env.SHOW_VIDEO_PATH;
  if (!outputPath) {
    throw new Error("SHOW_VIDEO_PATH is required");
  }
  ...
  const categories = page.getByRole("navigation").getByRole("link", {
    name: "Categories",
  });
  await moveTo(categories);
  await categories.click();
  await page.waitForSelector("h1", { state: "visible" });
  await page.waitForTimeout(500);

  const biotech = page.getByRole("link", { name: /Biotechnology/i }).first();
  await moveTo(biotech);
  await biotech.click();
  await page.waitForSelector("h1", { state: "visible" });
  await page.waitForTimeout(600);

  const article = page
    .getByRole("link", { name: /AI-Guided CRISPR/i })
    .first();
  await moveTo(article);
  await article.click();
  await page.waitForSelector("h1", { state: "visible" });
